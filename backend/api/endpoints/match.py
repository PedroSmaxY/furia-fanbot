...
def debug_results() -> Dict[str, Any]:
    ...
    try:
        ...
        for i, container in enumerate(
            result_containers[:2]
        ):  # Analisar apenas os 2 primeiros para não sobrecarregar
            # Data do grupo de resultados
            date_header = container.select_one(".standard-headline")
            date_text = (
                date_header.text.strip() if date_header else "Data não encontrada"
            )
            ...
            for j, match in enumerate(
                match_results[:3]
            ):  # Primeiros 3 resultados de cada data
                match_data = {
                    "index": j,
                    "html_structure": str(match)[:200]
                    + "...",  # Primeiros 200 caracteres
                    "teams": [team.text.strip() for team in match.select(".team")],
                    "score": (
                        match.select_one(".result-score").text.strip()
                        if match.select_one(".result-score")
                        else None
                    ),
                    "event": (
                        match.select_one(".event-name").text.strip()
                        if match.select_one(".event-name")
                        else None
                    ),
                    "map": (
                        match.select_one(".map-text").text.strip()
                        if match.select_one(".map-text")
                        else None
                    ),
        ...
        for selector in important_selectors:
            ...
            if elements:
                element_samples[selector] = {
                    "count": len(elements),
                    "example": (
                        str(elements[0])[:150] + "..." if elements else "Não encontrado"
                    ),
                }
        ...
        return {
            "status": "success",
            "data": {"debug_info": debug_info, "base_url": results_url},
        }
